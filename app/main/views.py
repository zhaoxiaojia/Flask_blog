...
def index():
    ...
    if show_followed:
        ...
        query = Post.query
    page = request.args.get('page', 1, type=int)  # type = int 确保参数无法转换成整数时返回默认值
    # query 返回集合 .paginate(三个参数）
    # page 对象 每页数量 error_out=False
...
def followed_by(username):
    ...
    return render_template('followers.html', user=user, title='Followers of', endpoint='.followed_by',
                           pagination=pagination, follows=follows)
